...
func runRefs(ctx context.Context, opts *RefsOptions, cliParams *settings.Run, ioStreams *terminal.IOStreams) error {
	...
	opts.KvxOutputFlags.AppName = cliParams.BinaryName + " get resolver refs"
	...
	if opts.KvxOutputFlags.FormatExplicit {
		// User explicitly requested table/list/tree -- show just the references
		return kvxOpts.Write(result.References)
	}
